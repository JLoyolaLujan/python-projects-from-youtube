...
ball2 = turtle.Turtle() #turtle object
ball2.speed(0)
ball2.shape("square")
ball2.color("white")
ball2.penup()
...
# movement (movement by pixel) - ball2

ball2.dx = -0.08 # delta x - x speed
ball2.dy = 0.08 # delta y 
...
while True :
    ...
    # move the ball2 
    ball2.setx(ball2.xcor() + ball2.dx) # get the x coord and add dx to it 
    ...
    if (ball2.xcor() > 340 and ball2.xcor() < 350) and (ball2.ycor() < paddle_b.ycor() + 50 and ball2.ycor() > paddle_b.ycor() - 50):
        ball2.setx(340) # bounce back to the left a bit
        ball2.dx *= -1
        winsound.PlaySound("ping.wav", winsound.SND_ASYNC)
    ...
    if (ball2.xcor() < -340 and ball2.xcor() > -350) and (ball2.ycor() < paddle_a.ycor() + 50 and ball2.ycor() > paddle_a.ycor() -50):
        ball2.setx(-340) # bounce back to the right a bit
        ball2.dx *= -1
        winsound.PlaySound("pong.wav", winsound.SND_ASYNC)

# AI player

    # if one ball is further to the right then it is followed by the paddle_b
